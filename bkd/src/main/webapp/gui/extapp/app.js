...
Ext.form.field.VTypes["phone"] = function(v){
	var testPhone = /^(\d{4}[-]?){1,2}(\d{2})$/;
	return testPhone.test(v);
};
Ext.form.field.VTypes["phoneMask"] = /[\d-]/;
Ext.form.field.VTypes["phoneText"] = 'Tel no hatalı.  Format 123-4567 veya 123-456-7890 olmalı';
...
Ext.onReady(function () {
    ...
    Ext.application({
        name: 'My',
        requires : [
            'My.controller.MainController',
            'My.controller.ihalepanel.IhalePanel',
            'My.controller.ihaletakip.IhaleTakip',
            'My.view.collection.Viewport'
        ],
        launch: function () {
            
            var mainController = Ext.create('My.controller.MainController'),
                borderPanel = mainController.getBorderPanel();
            
            Ext.create('My.view.collection.Viewport', {
                items : [borderPanel]
            });
